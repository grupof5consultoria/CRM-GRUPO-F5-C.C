...
import { BillingHistoryToggle } from "./BillingHistoryToggle";
...
const PAYMENT_LABELS: Record<string, string> = {
  ...
  bank_transfer: "Transf.", cash: "Dinheiro",
};
...
/**
 * Billing period rule (v3 — correct):
 * - paid        → month of paidAt
 * - pending AND dueDate is in the past (overdue / wrong date set) → current month
 * - everything else (pending future, cancelled, etc.)             → actual dueDate month
 */
function getChargePeriod(charge: {
  ...
  if (charge.status === "paid" && charge.paidAt) {
    const d = new Date(charge.paidAt);
    return { key: periodKey(d), label: periodLabel(d) };
  }
  if (charge.status === "pending" && new Date(charge.dueDate) < now) {
    return { key: periodKey(now), label: periodLabel(now) };
  }
...
export default async function BillingPage({ searchParams }: PageProps) {
  ...
  // Sort descending; split current+future vs past
  const sortedGroups = Array.from(grouped.entries()).sort((a, b) => b[0].localeCompare(a[0]));
  const activeGroups  = sortedGroups.filter(([k]) => k >= currentKey);
  const historyGroups = sortedGroups.filter(([k]) => k < currentKey);

  const historyTotalPaid = historyGroups.reduce(
    (s, [, g]) => s + g.charges.filter(c => c.status === "paid").reduce((a, c) => a + Number(c.value), 0),
    0
  );
            ...
            {charges.length === 0 ? (
              ...
              <>
                ...
                {activeGroups.map(([key, group]) => (
                  <MonthGroup key={key} groupKey={key} currentKey={currentKey} group={group} now={now} />
                ))}

                {/* Histórico recolhido */}
                {historyGroups.length > 0 && (
                  <BillingHistoryToggle monthCount={historyGroups.length} totalPaid={historyTotalPaid}>
                    {historyGroups.map(([key, group]) => (
                      <MonthGroup key={key} groupKey={key} currentKey={currentKey} group={group} now={now} />
                    ))}
                  </BillingHistoryToggle>
                )}
              </>
            )}
          </div>
...
function MonthGroup({
  ...
  groupKey: string;
  currentKey: string;
  group: { label: string; charges: Awaited<ReturnType<typeof getCharges>> };
  now: Date;
}) {
  const isCurrentMonth = groupKey === currentKey;
    ...
    <div className="bg-[#1a1a1a] rounded-2xl border border-[#262626] overflow-hidden">
      ...
      <div className={`px-5 py-3 border-b border-[#262626] flex items-center justify-between ${isCurrentMonth ? "bg-violet-600/10" : ""}`}>
        <div className="flex items-center gap-2">
          {isCurrentMonth && <span className="w-2 h-2 rounded-full bg-violet-400" />}
          <span className={`text-sm font-bold ${isCurrentMonth ? "text-violet-300" : "text-gray-400"}`}>
            {group.label}
            ...
            return (
              <TableRow key={charge.id}>
                ...
                <TableTd>
                  ...
                  {charge.isRecurring && <p className="text-xs text-violet-400 mt-0.5">🔄 Recorrente</p>}
                </TableTd>
                <TableTd>
