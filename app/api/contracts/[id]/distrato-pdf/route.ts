...
export async function GET(
  ...
  if (!contract || !contract.nomeContratante || !contract.distratoToken) {
  ...
  const html = buildHtml({
    title: `Distrato — ${contract.title}`,
    distratoText,
    isSigned,
    signedByName: contract.distratoSignedByName,
    signedByCpf: contract.distratoSignedByCpf,
    signedAt: contract.distratoSignedAt,
    signedIp: contract.distratoSignedIp,
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .print-bar { position: fixed; top: 0; left: 0; right: 0; background: #7f1d1d; padding: 10px 20px; display: flex; align-items: center; justify-content: space-between; z-index: 100; box-shadow: 0 2px 8px rgba(0,0,0,0.3); }
...
<body>
  ...
  <div class="page">
    ...
    ${p.isSigned
      ? `<div class="status-banner-signed">✓ <strong>Distrato assinado digitalmente</strong><br>Assinado por: <strong>${p.signedByName}</strong> (CPF: ${p.signedByCpf})<br>Data/Hora: ${signedDateStr}${p.signedIp ? ` — IP: ${p.signedIp}` : ""}</div>`
      : `<div class="status-banner">⚠ <strong>Aguardando assinatura do CONTRATANTE</strong></div>`
    }

    <pre class="contract-body">${p.distratoText.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</pre>
